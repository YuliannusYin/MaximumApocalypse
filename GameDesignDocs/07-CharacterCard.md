...
> 应用 v1 决策：D6（装备栏容量默认 4 / 狗 0）、D8（死亡后装备按来源处理）、D11（纠缠怪物处理）、D15（MVP 5 角色）、R1（饥饿状态切换）、R3（怪物纠缠）。
...
- **Entity 基类**：`Survivor extends Entity`，`Entity` 是 `Node` 子类（待 Entity 基类设计文档定义公共接口，如 `take_damage` / `heal` / `die` 等）。`Monster` / `MapBlock` 等也继承 `Entity`。
...
# === 生命与饥饿 ===
var current_hp: int
var current_hunger: int = 1                       # 初始饥饿等级 1
var is_hungry: bool = false                       # hunger ≥ 6 时 true（R1）
var hunger_damage_sequence: int = 2               # 下次饥饿伤害值（每次进入饥饿 +1，退出重置回 2）
...
# === 位置 ===
var current_location: MapBlock                    # 当前所在地块（见 MapBlock 文档）

# === 行动点 ===
var actions_remaining: int = 0                    # 本回合剩余行动点（标准行动点）
...
func discard_from_hand(card: CardInstance, pile: DiscardPile) -> void

# --- 移动 ---
# 移动到目标地块（触发 ON_MOVE 时机，可能触发地块效果）
func move_to(target_block: MapBlock) -> void

# --- 拾荒 ---
# 从地块关联的拾荒牌堆抓 1 张（pile_color 来自 MapBlock.scavenger_piles）
...
饥饿伤害序列（每次进入饥饿状态触发）：
  - 第 1 次进入饥饿：受 2 点伤害，hunger_damage_sequence = 3
  - 第 2 次进入饥饿：受 3 点伤害，hunger_damage_sequence = 4
  - 第 3 次进入饥饿：受 4 点伤害，hunger_damage_sequence = 5
  - ...（每次 +1）
  - 退出饥饿（hunger ≤ 5）时 hunger_damage_sequence 重置回 2

进入饥饿状态的当回合不扣血（R1）：
  - hunger 从 5 涨到 6 时，is_hungry = true，但本回合不再受饥饿伤害
  - 下次回合开始时若仍 hunger ≥ 6，按 hunger_damage_sequence 扣血
```

> 饥饿伤害通过 `ON_HUNGER_DAMAGE` 时机触发，可被技能修改（如消防员"能量饮料"免疫饥饿伤害到下回合）。
...
| 2 | `extra_actions`（野地夹克恢复的额外行动）与 `actions_remaining` 的消耗顺序 | 倾向先消耗 `extra_actions` 再消耗 `actions_remaining`（v1 第 8.2 节） |
...
| 8 | `current_location` 的初始值（游戏开始时所有玩家在起始地块） | 由任务卡 `initial_setup` 定义，GameSession 初始化时设置 |
