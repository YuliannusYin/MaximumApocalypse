...
# 注：地块上**没有**牌堆，拾荒牌堆是全局的 `ScavengerDeckStack`。
#     地块只记录可拾荒的颜色入口 `scavenger_piles`（data / override）。
...
# 该地块当前是否可拾荒（任一可拾荒颜色对应的全局牌堆非空）
func can_scavenge() -> bool:
    for color in get_scavenger_piles():
        if not game_session.scavenger_deck_stack.is_empty(color):
            return true
    return false
...
> 设计取舍：地块不持有拾荒牌堆，只作为颜色入口。拾荒时从全局 `ScavengerDeckStack` 抓牌（见 10-Deck.md），地块标记不会被消耗。
...
多色地块（如机场 `[RED, GREEN]`）：玩家拾荒时可选择任一全局牌堆非空的颜色抓 1 张。
...
地块不持有拾荒牌堆，只通过 `scavenger_piles`（或 `scavenger_piles_override`）记录该地块可以拾荒的颜色入口。拾荒牌堆是全局唯一的 `ScavengerDeckStack`（见 10-Deck.md），由 GameSession 在游戏开始时初始化，所有地块共享。

```text
# 地块无需初始化牌堆；scavenger_piles 来自 MapBlockData 或 instances_config override
# GameSession 只需保证全局 ScavengerDeckStack 已按任务卡配置初始化
```

> 设计取舍：地块标记只是颜色入口，不会被消耗。玩家每次拾荒从全局 `ScavengerDeckStack` 抓 1 张牌（D112）。

### 9.2 拾荒行动定义

| 行动类型 | 消耗 | 触发 | 说明 |
|---|---|---|---|
| 标准拾荒 | 1 行动点 | 玩家主动 | 从当前地块任一可拾荒颜色对应的全局牌堆抓 1 张拾荒卡到手牌 |
| 免费拾荒 | 0 行动点 | `free_scavenge_once` 效果 | 不消耗行动点，从当前地块任一可拾荒颜色对应的全局牌堆抓 1 张 |

- 多色地块：玩家弹窗选择 1 个全局牌堆非空的颜色
- 某色全局牌堆空时该色不可拾（D12）
- 地块所有可拾荒颜色对应的全局牌堆都空时 `can_scavenge()` 返回 false，地块拾荒标记 UI 变灰
...
> 实现要点：`EffectHandler` 实现 `free_scavenge_once` 时不消耗行动点，从 `get_scavenger_piles()` 中选择一个全局牌堆非空的颜色抓 1 张拾荒卡。
...
### Q5. ✅ 已确认：地块是拾荒颜色入口（D112）

地块不持有拾荒牌堆。玩家在有 `scavenger_piles` 的地块上，消耗 1 点行动即可从全局 `ScavengerDeckStack` 对应颜色牌堆抓 1 张拾荒卡；地块标记只表示颜色入口，不会被消耗。
