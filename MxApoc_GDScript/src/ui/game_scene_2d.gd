...
@onready var _discard_button: Button = $UILayer/DiscardButton
@onready var _end_turn_button: Button = $UILayer/EndTurnButton
...
var _settlement_area: CardSettlementArea
...
func _start_game_flow() -> void:
	...
	_build_hand_and_settlement_areas()
...
func _build_hand_and_settlement_areas() -> void:
	_settlement_area = CardSettlementArea.new()
	_ui_layer.add_child(_settlement_area)
	_hand_area = HandDisplayArea.new()
	_hand_area.set_settlement_area(_settlement_area)
	_hand_area.card_used.connect(_on_hand_card_used)
...
func _on_hand_card_used(card: Variant) -> void:
...
## 仅当当前玩家处于行动阶段且行动次数归零时显示“结束回合”按钮。
func _refresh_end_turn_button() -> void:
	var current: Variant = Game.get_current_player()
	var can_end: bool = false
	if current != null and is_instance_valid(current) and current.get("in_phase") == "action":
		can_end = current.get("action_count") <= 0
	_end_turn_button.visible = can_end
...
func _on_action_requested(_player: Variant) -> void:
	# 行动阶段：玩家通过 UI 操作（主动技能按钮、手牌使用、结束回合）
...
func _on_choose_target_requested(n: int) -> void:
	var targets: Array = []
	...
	var current: Variant = Game.get_current_player()
	if current != null and is_instance_valid(current):
		for m in current.get("monster_zone"):
			if m != null and is_instance_valid(m):
				targets.append(m)
	_show_target_select_popup(targets, n)
...
## 目标选择弹窗（choose_target）：选择 N 个目标
func _show_target_select_popup(targets: Array, n: int) -> void:
	if targets.is_empty():
	...
	var panel := Panel.new()
	panel.position = Vector2(515, 180)
	panel.size = Vector2(400, 60 + targets.size() * 38)
	overlay.add_child(panel)
	...
	for target in targets:
		...
		btn.add_theme_font_size_override("font_size", 13)
		btn.pressed.connect(_on_target_select_clicked.bind(target, btn))
		vbox.add_child(btn)
	...
	ok_btn.custom_minimum_size = Vector2(140, 30)
	ok_btn.disabled = true
	ok_btn.pressed.connect(_on_target_select_confirmed)
	hbox.add_child(ok_btn)
	_popup_ok_button = ok_btn
	...
	cancel_btn.text = "取消"
	cancel_btn.custom_minimum_size = Vector2(80, 30)
	cancel_btn.pressed.connect(_on_target_select_cancelled)
	hbox.add_child(cancel_btn)


func _on_target_select_clicked(target: Variant, btn: Button) -> void:
...
func _on_target_select_cancelled() -> void:
	_close_popup()
	_gui_input.respond_choose_target([])
...
func _show_discard_pile_popup() -> void:
	var overlay := _create_modal_overlay()
	var scavenge_cards: Array = []
	if Game.scavenge_discard_pile != null and is_instance_valid(Game.scavenge_discard_pile):
		scavenge_cards = Game.scavenge_discard_pile.get("cards")
	var monster_cards: Array = []
	if Game.monster_discard_pile != null and is_instance_valid(Game.monster_discard_pile):
		monster_cards = Game.monster_discard_pile.get("cards")

	var panel := Panel.new()
	panel.position = Vector2(315, 80)
	...
	var title := Label.new()
	title.text = "弃牌堆（拾荒 %d 张 / 怪物 %d 张）" % [scavenge_cards.size(), monster_cards.size()]
	title.add_theme_font_size_override("font_size", 14)
	vbox.add_child(title)
	...
	if not scavenge_cards.is_empty():
		var h1 := Label.new()
		h1.text = "— 拾荒弃牌堆 —"
		h1.add_theme_font_size_override("font_size", 12)
		content.add_child(h1)
		for card in scavenge_cards:
			var lbl := Label.new()
			lbl.text = "  %s（%s）" % [card.get("card_name"), card.get("card_type")]
	...
	if scavenge_cards.is_empty() and monster_cards.is_empty():
		var empty := Label.new()
		empty.text = "弃牌堆为空"
