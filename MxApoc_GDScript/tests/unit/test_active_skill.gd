...
func _make_revealed_block(block_name: String = "test_block", x: int = 0, y: int = 0) -> MapBlock:
...
func test_use_active_skill_block_target() -> void:
	var p: Player = _make_combat_player()
	var b1: MapBlock = _make_revealed_block("b1", 0, 0)
	var b2: MapBlock = _make_revealed_block("b2", 1, 0)
...
func test_use_active_skill_entity_target() -> void:
	var p: Player = _make_combat_player()
	var p2: Player = _make_combat_player()
