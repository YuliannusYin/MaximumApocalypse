...
func test_trigger_filter_false_skips_content() -> void:
	...
	var s: Skill = Skill.new()
	s.trigger = "on_take_damage"
	s.filter = func(_p, _t, _ev, _g) -> bool:
		return false
	s.content = func(_p, _t, _ev, _g) -> void:
		called.append(true)
	e.add_skill(s)
