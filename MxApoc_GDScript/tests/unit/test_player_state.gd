...
func test_recover_normal() -> void:
	...
	player.set_hp(3)
	player.recover(2)
	assert_eq(player.get_hp(), 5, "HP=3 + 2 = 5")
...
func test_recover_exceeds_max_clamped() -> void:
	...
	player.set_hp(5)
	player.recover(5)
	assert_eq(player.get_hp(), 6, "clamp 到 max=6")
...
func test_recover_full_hp_noop() -> void:
	...
	player.set_hp(6)
	player.recover(3)
	assert_eq(player.get_hp(), 6, "已满血,max_recover=0,不增加")
...
func test_recover_zero_or_negative_noop() -> void:
	...
	player.set_hp(3)
	player.recover(0)
	assert_eq(player.get_hp(), 3, "recover(0) 不变")
	player.recover(-1)
	assert_eq(player.get_hp(), 3, "recover(-1) 不变")
...
func test_recover_triggers_recover_during_hook() -> void:
	...
	player.set_max_hp(6)
	player.set_hp(3)
	player.add_skill(Skill.make("回复生命时", Callable(), func(event: Event) -> void: event.num += 1))
	player.recover(2)
	# event.num=2 → 钩子 +1 → event.num=3 → max_recover=3 → 不 clamp → 增加生命值(3) → HP=6
	assert_eq(player.get_hp(), 6, "钩子 +1 后 clamp 到 max-HP=3,实际 +3")


func test_recover_triggers_before_and_after_hooks() -> void:
	var player := Player.new()
	player.set_hp(3)
	player.add_skill(_make_recording_skill("回复生命前", "前"))
	player.add_skill(_make_recording_skill("回复生命后", "后"))
	player.recover(2)
...
func test_recover_cancel_in_recover_during() -> void:
	var player := Player.new()
	player.set_hp(3)
	player.add_skill(_make_cancel_skill("回复生命时"))
	player.add_skill(_make_recording_skill("回复生命后", "后"))
	player.recover(2)
	assert_eq(player.get_hp(), 3, "cancel 后不增加")
...
func test_recover_negative_after_hook_clamped_to_zero() -> void:
	...
	player.set_max_hp(6)
	player.set_hp(3)
	player.add_skill(Skill.make("回复生命时", Callable(), func(event: Event) -> void: event.num = -5))
	player.recover(2)
	# event.num=-5,增加生命值(-5) 因 n<=0 不变更
	assert_eq(player.get_hp(), 3, "钩子改 num=-5,add_hp 拒绝负数,HP 不变")
...
func test_increaseHunger_below_6_only_increases() -> void:
	...
	assert_eq(player.get_hunger(), 5, "hunger=2+3=5")
	assert_eq(player.countMark("饥饿伤害等级"), 0, "无饥饿伤害标记")
	assert_eq(player.get_hp(), 6, "HP 不变")
...
func test_increaseHunger_at_6_flips_role_card() -> void:
	...
	assert_false(player.get_role_card().is_front(), "角色卡反面")
	assert_eq(player.countMark("饥饿伤害等级"), 1, "1 层标记")
	assert_eq(player.get_hp(), 4, "level 1 扣 2,HP=6-2=4")
...
func test_increaseHunger_at_6_already_flipped() -> void:
	...
	player.increaseHunger(1)
	assert_false(player.get_role_card().is_front(), "角色卡仍反面(不重复翻)")
	assert_eq(player.countMark("饥饿伤害等级"), 1, "1 层标记")
	assert_eq(player.get_hp(), 4, "level 1 扣 2")
...
func test_increaseHunger_level_2_deals_4_damage() -> void:
	...
	player.increaseHunger(1)
	assert_eq(player.countMark("饥饿伤害等级"), 2, "2 层")
	assert_eq(player.get_hp(), 6, "level 2 扣 4,HP=10-4=6")


func test_increaseHunger_level_3_deals_6_damage() -> void:
	var player := Player.new()
	player.set_hp(20)
	player.set_max_hp(20)
	player.set_hunger(6)
	player.get_role_card().flip()
	player.addMarkSkill("饥饿伤害等级", 2)
	player.increaseHunger(1)
	assert_eq(player.countMark("饥饿伤害等级"), 3, "3 层")
	assert_eq(player.get_hp(), 14, "level 3 扣 6,HP=20-6=14")
...
func test_increaseHunger_level_4_deals_8_damage() -> void:
	...
	player.set_hunger(6)
	player.get_role_card().flip()
	player.addMarkSkill("饥饿伤害等级", 3)
	player.increaseHunger(1)
	assert_eq(player.countMark("饥饿伤害等级"), 4, "4 层")
	assert_eq(player.get_hp(), 12, "level 4 扣 8,HP=20-8=12")
...
func test_increaseHunger_multi_point_iteration() -> void:
	...
	assert_eq(player.get_hunger(), 6, "hunger=4+3 点 = 6")
	assert_eq(player.countMark("饥饿伤害等级"), 1, "1 层标记")
	assert_false(player.get_role_card().is_front(), "角色卡反面")
	assert_eq(player.get_hp(), 4, "level 1 扣 2,HP=6-2=4")
...
func test_increaseHunger_zero_or_negative_noop() -> void:
	...
	assert_eq(player.get_hunger(), 2, "increaseHunger(0) 不变")
	player.increaseHunger(-1)
	assert_eq(player.get_hunger(), 2, "increaseHunger(-1) 不变")
...
func test_decreaseHunger_normal() -> void:
	var player := Player.new()
	player.set_hunger(4)
	var result := player.decreaseHunger(2)
	assert_eq(player.get_hunger(), 2, "hunger=4-2=2")
...
func test_decreaseHunger_floor_at_1() -> void:
	var player := Player.new()
	player.set_hunger(2)
	var result := player.decreaseHunger(5)
	assert_eq(player.get_hunger(), 1, "clamp 到 1")
...
func test_decreaseHunger_already_at_1_returns_false() -> void:
	var player := Player.new()
	player.set_hunger(1)
	var result := player.decreaseHunger(1)
	assert_eq(player.get_hunger(), 1, "hunger 仍 1")
...
func test_decreaseHunger_clears_hunger_marks() -> void:
	...
	assert_eq(player.get_hunger(), 5, "hunger=6-1=5")
	assert_eq(player.countMark("饥饿伤害等级"), 0, "标记清除")
...
func test_decreaseHunger_flips_role_card_back() -> void:
	...
	player.get_role_card().flip()
	player.decreaseHunger(1)
	assert_true(player.get_role_card().is_front(), "翻回正面")


func test_decreaseHunger_zero_or_negative_noop() -> void:
	var player := Player.new()
	player.set_hunger(3)
	var result := player.decreaseHunger(0)
	assert_eq(player.get_hunger(), 3, "decreaseHunger(0) 不变")
...
func test_poison_no_mark_noop() -> void:
	...
	player.set_hp(6)
	player.poison()
	assert_eq(player.get_hp(), 6, "无 poison 标记,HP 不变")


func test_poison_deals_damage_equal_to_mark_level() -> void:
	var player := Player.new()
	player.set_hp(6)
	player.addMarkSkill("poison", 3)
	player.poison()
	assert_eq(player.get_hp(), 3, "poison 3 层,扣 3,HP=6-3=3")


func test_poison_uses_no_source_damage() -> void:
	var player := Player.new()
	player.set_hp(10)
	player.addMarkSkill("poison", 2)
	# 挂"造成伤害前"(source 侧钩子),source=null 不应触发
	player.add_skill(_make_recording_skill("造成伤害前", "S前"))
	player.poison()
...
func test_poison_damage_type_is_poison() -> void:
	...
	player.add_skill(Skill.make("受到伤害时", Callable(), func(event: Event) -> void: captured_type.append(event.type)))
	player.poison()
