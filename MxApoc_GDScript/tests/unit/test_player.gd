...
func test_move_to_null_target_returns_false() -> void:
	...
	Game.map_area = [src]
	p.current_block = src
	var result: bool = p.move_to(null)
	# null 目标不应崩溃；具体行为依实现
...
func test_consume_charge_basic() -> void:
	...
	var e: EquipmentCard = _make_equipment("weapon")
	e.charge_current = 3
	var result: bool = p.consume_charge(e, 2)
	assert_true(result)
...
func test_consume_charge_insufficient_returns_false() -> void:
	...
	var e: EquipmentCard = _make_equipment("weapon")
	e.charge_current = 1
	var result: bool = p.consume_charge(e, 3)
...
func test_consume_charge_cancel_before() -> void:
	...
	var e: EquipmentCard = _make_equipment("weapon")
	e.charge_current = 3
	var result: bool = p.consume_charge(e, 1)
	assert_false(result)
...
func test_choose_delegates_to_input() -> void:
	...
	p.input = cli
	var result: Variant = p.choose(["a", "b", "picked"])
...
func test_discard_non_boss_monster_to_mark() -> void:
	...
	p.discard_non_boss_monster_to_mark()
