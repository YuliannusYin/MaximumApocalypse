...
func test_wait_action_returns_response() -> void:
	...
	_respond(input, "respond_action", "end_turn")
	await get_tree().create_timer(0.02).timeout
...
func test_wait_action_returns_null_by_default() -> void:
	...
	_run_wait_action(input, holder)
	await get_tree().create_timer(0.02).timeout
	_respond(input, "respond_action", null)
	await get_tree().create_timer(0.02).timeout
...
func test_choose_returns_selected_option() -> void:
	...
	await get_tree().create_timer(0.02).timeout
	_respond(input, "respond_choose", "选项B")
	await get_tree().create_timer(0.02).timeout
...
func test_choose_card_returns_array() -> void:
	...
	_respond(input, "respond_choose_card", cards)
	await get_tree().create_timer(0.02).timeout
...
func test_choose_card_returns_empty_on_non_array() -> void:
	...
	await get_tree().create_timer(0.02).timeout
	_respond(input, "respond_choose", "not_an_array")
	await get_tree().create_timer(0.02).timeout
...
func test_choose_target_returns_array() -> void:
	...
	_respond(input, "respond_choose_target", targets)
	await get_tree().create_timer(0.02).timeout
...
func test_choose_map_block_returns_block() -> void:
	...
	_respond(input, "respond_choose_block", block)
	await get_tree().create_timer(0.02).timeout
...
func test_confirm_returns_bool() -> void:
	...
	await get_tree().create_timer(0.02).timeout
	_respond(input, "respond_confirm", true)
	await get_tree().create_timer(0.02).timeout
...
func test_confirm_returns_false() -> void:
	...
	_run_confirm(input, holder, "确认？")
	await get_tree().create_timer(0.02).timeout
	_respond(input, "respond_confirm", false)
	await get_tree().create_timer(0.02).timeout
...
func test_nested_request_preempts_wait_action_and_restores_it() -> void:
	...
	_respond(input, "respond_choose_card", [])
	...
	_respond(input, "respond_action", "move")
	await get_tree().create_timer(0.05).timeout
...
func test_single_request_flow_compatible() -> void:
	...
	_run_wait_action(input, holder)
	await get_tree().create_timer(0.02).timeout
	_respond(input, "respond_action", "attack")
	await get_tree().create_timer(0.02).timeout
...
func test_fire_and_forget_owner_does_not_leak_to_next_request() -> void:
	...
	assert_eq(owners, [owner, "__system__"], "单向通知消耗 owner 后不应污染下一次请求")
	_respond(input, "respond_confirm", false)
	await get_tree().create_timer(0.02).timeout
...
func _respond(input: GUIPlayerInput, method: String, value: Variant) -> void:
	input.call(method, value, input.get_active_request_id(), input.get_active_request_owner())
