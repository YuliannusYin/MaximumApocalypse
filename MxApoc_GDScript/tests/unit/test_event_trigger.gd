...
func test_trigger_calls_matching_skills_in_order() -> void:
	var e := Entity.new()
	e.add_skill(_make_recording_skill("受到伤害时", "A"))
	e.add_skill(_make_recording_skill("受到伤害时", "B"))
	e.add_skill(_make_recording_skill("受到伤害时", "C"))
	var event := Event.new()
	e.trigger("受到伤害时", event)
...
func test_trigger_skills_with_false_filter_not_called() -> void:
	var e := Entity.new()
	e.add_skill(Skill.make("受到伤害时", func(_ev: Event) -> bool: return false, _record_callable("A")))
	e.add_skill(_make_recording_skill("受到伤害时", "B"))
	var event := Event.new()
	e.trigger("受到伤害时", event)
...
func test_trigger_matches_skill_with_multiple_trigger_names() -> void:
	var e := Entity.new()
	e.add_skill(_make_recording_skill("游戏开始时、受到伤害时", "X"))
	var event := Event.new()
	e.trigger("受到伤害时", event)
	assert_eq(_call_log, ["X"], "复合 trigger 的子串应匹配")
	_call_log.clear()
	e.trigger("游戏开始时", Event.new())
	assert_eq(_call_log, ["X"], "复合 trigger 的另一子串也应匹配")
	_call_log.clear()
	e.trigger("造成伤害时", Event.new())
...
func test_cancel_stops_subsequent_skills() -> void:
	var e := Entity.new()
	e.add_skill(_make_recording_skill("受到伤害时", "A"))
	e.add_skill(_make_cancel_skill("受到伤害时", "B"))
	e.add_skill(_make_recording_skill("受到伤害时", "C"))
	var event := Event.new()
	e.trigger("受到伤害时", event)
...
func test_trigger_name_set_before_loop() -> void:
	...
	e.add_skill(Skill.make("受到伤害时", Callable(), func(ev: Event) -> void: captured.append(ev.trigger_name)))
	var event := Event.new()
	e.trigger("受到伤害时", event)
...
func test_skill_content_can_modify_event_num() -> void:
	var e := Entity.new()
	e.add_skill(Skill.make("受到伤害时", Callable(), func(ev: Event) -> void: ev.num += 5))
	var event := Event.new()
	event.num = 10
	e.trigger("受到伤害时", event)
