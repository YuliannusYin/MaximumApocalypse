...
> 状态: `[x] 已完成`(代码 + 测试,§5 第 8 项 UI 验证待用户)
>
...
5. **Player.roll_two_dice()**:待定义方法,本轮实现
...
- **代码**: 01 轮 EventTrigger、02 轮 Player(get_sneak/get_current_block/get_role_card)
...
```
- 投骰子得 result
- 找所有已展示且 monster_spawn_value == result 的地图块
- 标记 < 3:+1 标记
...
- 命名风格:属性 `monster_spawn_value` 用 snake_case;方法保留设计文档的 camelCase 形式(countMonsterMark/addMonsterMark/hasPlayer/countMonster)
- addPlayer/removePlayer 参数用 Variant,规避 Player↔MapBlock 循环依赖
...
### 4.4 Player.get_current_block() 扩展
02 轮 `get_current_block()` 返回 null。本轮扩展:
```gdscript
# Player 类追加
...
设计文档:"所有已经展示的,且 monster_spawn_value 等于 result 的地图块"。本轮无 game 对象:
...
| `roll_two_dice` | snake_case | 设计文档中文方法名"随机投掷两颗大骰子"的英文译名(AGENTS.md §3.6) |
| `countMonsterMark`/`addMonsterMark`/`hasPlayer`/`countMonster` | 英文 camelCase | 设计文档伪代码用此形式,且为 MapBlock 接口 |
| `monster_spawn_value` | snake_case 属性 | 设计文档"怪物生成点数"的英文译名(AGENTS.md §3.6) |
...
├── player.gd          # 02-04 轮 + 追加 judge/sneakJudge/monsterSpawnJudge/roll_two_dice/drawMonster stub/set_current_block
├── dice.gd            # 05 轮新增
...
1. [x] 与用户确认 §4.3(MapBlock 方法)、§4.5(revealed_blocks 参数)、§4.7(仅返回 bool)、§7(Dice 注入 Callable)
2. [x] 新建 `scripts/system/dice.gd`(§4.1)
3. [x] 新建 `scripts/system/map_block.gd`(§4.2)
4. [x] 在 `scripts/system/player.gd` 追加:
   - `roll_two_dice()`(§4.1)
   ...
   - `set_dice_roller()`(测试用 Callable 注入)
5. [x] 在 `待定义方法.md` §10.3 登记 `drawMonster` 为 stub 状态;§9.19 登记 monsterSpawnJudge 签名变更
6. [x] 新建 `tests/unit/test_judge.gd`(§6 验收用例,21/21 通过)
7. [x] 运行 GUT 测试,全部通过(109/109:test_event_trigger 10 + test_player 28 + test_damage_flow 21 + test_player_state 29 + test_judge 21)
8. [ ] 走通 [AGENTS.md](../AGENTS.md) §6.2 关键路径 1-3,确认未破坏 UI(用户手动验证)
...
- `test_judge_uses_roll_two_dice`: 注入 `set_dice_roller` Callable,断言 judge 调用了它一次
...
- `test_sneakJudge_with_null_block_treats_as_zero`: get_current_block()=null,潜行值=8(sneakValue=8,无减成),mock 骰子返回 8 → true
- `test_sneakJudge_negative_sneakValue`: 潜行值=2,地块 5 怪物(sneakValue=-3),mock 骰子返回 2 → false(2 > -3)

### 6.3 monsterSpawnJudge
- `test_monsterSpawnJudge_adds_mark_when_below_3`: 1 个已展示地图块,monster_spawn_value=7,countMonsterMark=0,mock 骰子返回 7 → 该块 countMonsterMark==1
...
- `test_monsterSpawnJudge_skips_non_matching_blocks`: 地块 monster_spawn_value=5,mock 返回 7 → 该块不变
...
### 6.5 Player 集成
- `test_player_set_current_block`: `set_current_block(block)` → `get_current_block()` == block
