...
- 技能描述：选择一张拾荒牌和同地图块内另一玩家，将该拾荒牌向该玩家展示，其可以选择一张拾荒牌与你交易
- 英文名：`trade`
- 可用阶段：行动阶段
- 可用次数：每回合 1 次（免费行动，不消耗行动次数）
- 可用条件：处于行动阶段，自己持有至少 1 张拾荒牌，且当前地块存活玩家数大于 1
- 卡牌选择：1 张
- 卡牌筛选：`source == "scavenge"`（手牌区或装备区）
- 位置：空字符串（手牌 + 装备）
- 目标选择：1 个
- 目标范围：短距离（同地块内）
- 目标筛选：目标不是玩家自身、是存活玩家，且目标持有至少 1 张拾荒牌
- 效果：`actions.trade_scavenge(player, target, event["cards"][0])` → `Player.trade_scavenge_with`
  1. 向目标展示所选拾荒牌（`show_card`，联机走 `GAME_EVENT`，不等 ACK）
  2. 询问目标是否同意；拒绝或目标取消选牌时 `EventSystem.cancel(event)`，**不** `record_use`，本回合可再向其他人发起
  3. 若同意，目标从自己的手牌或装备区选择 1 张拾荒牌
  4. 双方先抽出再放入：尽量保持原区域（手牌仍进手牌，装备仍装备）；装备栏不足或同名冲突（燃料除外）时该张落入对方手牌
  5. 不得走 `discard()`（任务 `card_discard_watch` 会误伤科学家）
- AI：`order: 0`，评分器恒 0，不主动发起；被询问时一律同意
