...
- 移动：有效行动点 > 0 且 **怪物区为空** 时当前地块相邻格；**例外**：当前格是旷野时即使有纠缠怪物也枚举走到**非旷野**邻格（怪跟玩家走，规则允许）。只给**严格靠近**行进目标的邻格正分（离开旷野不要求更近）。远近用 `path_distance_to`（沿邻格 BFS），不是曼哈顿，否则任务 0 在 (2,1) 东侧空洞处所有邻格都会被判成更远而原地抽牌
- 抓牌：有效行动点 > 0 且牌堆非空；拾荒只枚举 **当前地块 `scavenge_colors`**（与 GUI 一致）。纠缠中或当前格是旷野时抓牌分 ≤ 0
...
`AIPlayerInput` 决策对齐 `CliPlayerInput`。`think_seconds` 默认 `0`（测试不卡）；场景里设 `0.4`。同一行动指纹连续 3 次则**跳过该选项再选次优**，不直接结束回合。卡牌空放（打出后仍在手）与展示类技能空放（行动点未变）本回合拉黑该指纹。`choose_block_inline`：最高分 ≤ 0 或格子不比当前更靠近行进目标则返回空（耐力 / 摩托车后续步停走）。当前在旷野时改为只选非旷野格，不要求更近。prompt 含「无人机」时改用清标记分，不要求更靠近行进目标。
...
**地图块技能** 可另写 `"hazard": N`（旷野约 4，只表示经过不舒服，不封路）。`score_block` 减去该值，并按怪物标记数 × 2.5 惩罚。旷野用名字 / `wilderness` 技能识别为必须离开的格：可以踏进再踏出，有效行动点 ≤ 1 时走进旷野 ≤ 0；已在旷野上离开分约 +40（可压过未斩杀攻击与击晕），不要求更靠近行进目标；若剩余行动能杀光自己面前的怪且还剩 ≥1 点再离开，允许先打。当前在旷野时拾荒 / 抽牌 / 非武器装备 / 升级 / 制衡等闲着 ≤ 0。
...
`damage` `heal` `food` `fuel` `ammo` `equip` `move` `draw` `stealth` `mission` `aoe` `weapon` `reveal` `grant_action` `hunger_ap` `pull` `skip_hunger` `stun` `team_buff` `buff`

- `aoe`：对射程内怪物求和（含致死加权）；会打到队友则按溅射扣分。同格 2+ 怪时压过单目标武器
- `stun`：击晕。清不完且回合结束会挨打时，按防住的攻击力计分
- `team_buff`：检查武器等全队增益。按仍纠缠的求生者人数计分，有人开战时先拍
- `buff`：升级等单件强化。纠缠中当闲着降权，不和攻击抢行动
...
| 9 | 攻击（武器技能、拳打） |
| 8 | 装备武器 / 关键防具；检查武器（全队 +1 伤） |
| 7 | 装填弹药 / 燃料 |
| 6 | 治疗；升级（武器 +1 伤，不当攻击） |
| 5 | 食物（饥饿由评分器按**目标**再加分）；灭火器等击晕 |
...
- `damage`：最终分 ≈ `effect.player + (-attitude(target)) * |effect.target|` + 威胁；打怪物为正，打队友 / 自己为负。**行动分**另加致死（预估伤害 ≥ 当前 hp）。`select_target < 0` 的 `aoe` 对怪物求和
...
- `stun`：2+ 怪且剩余行动打不死时，按将挨到的攻击力计分
- `team_buff`：有人纠缠才为正；人数越多越先拍
- 其它：`effect.player + attitude(target) * effect.target`

卡级 `order == 0` 时，出牌评分回退到该牌主动技能的 `order`。

**有可用已装备武器（`weapon`+`damage` 且 filter 通过）时拳打 ≤ 0。** 装备栏按 **size 占用** 判断是否挤装；非武器且会超栏时重罚。溢出时若按 useful 从低到高丢掉的牌包含任务物资，**没战时该装备行动 ≤ 0**；**纠缠中允许丢物资换武器，但优先丢非武器。** 有纠缠怪物且当前没有可用的已装备武器攻击时，手里的武器装备加局势分，压过徒手攻击，先花 1 点装上再打。已有可用武器时不再为换另一把单目标武器加分（类固醇也不会让猎人卸弩装弓）；同格 2+ 怪且栏位够时，AOE 武器（打火机）仍可加分装上。**有效行动点 ≤ 1 时不加换装加分**，最后 1 点出伤害而不是换装；最后 1 点也不走进有怪物标记、河流或旷野的格子。已装备同名武器再装同名 ≤ 0（打断维修→炮塔环）。`升级` / `检查武器` **不标 `damage`**，避免当攻击或打自己。齐射倒空弹药，仅在能打死高血量怪时考虑。`加油` 在任务仍缺该物资族时降权（允许给载具加，但不抢交任务）。自制子弹 / 阅读使用说明等以装备为目标时，任务物资给 -99。制衡按即将丢掉的两张 useful 计分；丢掉武器 / 治疗 / 食物 / 任务物时再扣。非任务需求且 useful 虚高的道具（如多余配件）在留牌时封顶，避免压过食物。伪装不按 `damage` 计；有可用已装备武器、或徒手能打死时不用。无人机选格按标记数加分，不复用走路的「躲标记」分。
...
- **抱团**：领队是持有任务物资的信使，否则是到个人行进目标最近的活人。其他人的行进目标改成领队所在格（领队在旷野上则改成其安全邻格），避免分头采集。走完后到最近队友图距离 > 2 则该步 ≤ 0；已离领队 ≥ 2 的人靠近加分、再拉大 ≤ 0；领队若这一步会把到最后一名的距离拉过 2 则 ≤ 0。不要求叠在同一格
...
- 有纠缠怪物时不枚举主动移动，**旷野除外**（可带着怪离开）
- 食物：饥饿 ≥ 3 即加分，≥ 5 再加一档；看目标不看使用者。饥饿 ≥ 5 时抽游戏牌 ≤ 0，含食物的拾荒堆加分。**纠缠中拾荒 / 抽游戏牌 ≤ 0**
- 任务仍缺燃料时 `加油` 降权，不禁止
