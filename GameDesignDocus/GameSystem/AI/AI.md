...
- 手牌：`player.is_card_usable`；伤害牌在合法目标只剩求生者时不枚举
...
| `useful` | 留牌价值 ∈ **[0, 100]**（弃牌 / 选牌时越高越留；第零轮按手牌均分决定是否重调）。技能可写 `0` |
| `tags` | 供局势加成。见下表 |
...
**怪物包 JSON** 每张怪物卡写 `"ai": { "threat": N }`（0–100）。默认公式：`clamp(等级基数 + max_hp + attack_damage * 2, 0, 100)`，基数 boss 55 / elite 35 / normal 15。`MonsterCard.instantiate` 拷到 `Monster.ai_threat`。伤害选目标：`score = threat + (预估伤害 ≥ hp ? 50 : 0) + 己方怪物区加成`；平手优先能击杀的。
...
- `wait_redraw_decision`：手牌平均 useful < 70 则重调；≥ 70 停止。本局最多 20 次，避免牌库全低价值时卡死开局
