...
每个地块持有一个二维坐标 `(x, y)`，对应任务地图要求二维数组中的位置（`map[y][x]`）。相邻关系基于**四向**（上下左右），不含对角线。战斗射程使用**曼哈顿距离** `|x1-x2| + |y1-y2|`。玩家行进（含 AI 贪心一步）使用沿相邻存活地块的 **BFS 图距离** `path_distance_to`，以便绕过 `no_block` 空洞与已摧毁格。
...
`path_distance_to(other) -> int`：沿四向相邻存活地块的最短路（BFS）。用于 AI / 行进「是否更靠近目标」。不可达返回 99；若当前格没有任何邻居（测试夹具未接入地图），退回曼哈顿以免旧单测失效。
