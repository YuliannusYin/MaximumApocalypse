...
| `find_by_net_id` | `static func find_by_net_id(game: Variant, net_id: int) -> Variant` | 按 net_id 线性查找实体（玩家/子生命体/手牌/装备/怪物/牌堆/地块） |
...
普通求生者 `bodies` 为空数组。老兵与狗等双子座位每项：

`{ "net_id", "english_name", "hp", "max_hp", "hunger", "alive", "is_front_side", "marks" }`。

`marks` 只含身体上的标记（如 `hunger_damage_level`）；座位级标记仍在玩家行 `marks`。`apply` 若座位尚无 `bodies`，用该座 survivor 调 `setup_companion_bodies`，再按 `english_name` 对齐并 stamp `net_id`。
