...
> 职责：JSON 技能与 `Entity.trigger` 使用的 **GameEvent 工厂与取消机制**。
> 类名 `EventSystem`，继承 `RefCounted`。
> **纯静态工具类，无字段，无实例**。所有方法均为 `static func`。
> trigger 方法定义在 [Entity.md](Entity.md) 的 `Entity.trigger()`。
> **领域操作入栈、输入请求、GameEvent 运行栈见 [EventScheduler.md](EventScheduler.md)**（本章不描述调度）。
...
> 工厂返回 `GameEvent`。JSON 仍按 Dictionary 字段名读写（值在 `data` 或节点属性）。字段键名统一 snake_case。

| 字段名 | 类型 | 说明 |
|------|------|------|
| `trigger_name` | String | 由 `set_trigger_name()` 设置，当前触发的 trigger 名 |
| `cancelled` | bool | 是否已取消 |
| `cancel` | Callable | `event["cancel"].call()` 或 `EventSystem.cancel(event)` |
| `id` | int | 节点 id |
| `root` | int | 树根事件 id |
| `children` | Array | 子 `GameEvent` 列表 |
| `actions` | GameActions | content 执行期间注入的操作门面，见 [EventScheduler.md §七](EventScheduler.md) |
...
- 调用 `event["cancel"].call()` 或 `EventSystem.cancel(event)` 后节点为 `CANCELLED`
...
创建通用 `GameEvent`：写入 `trigger_name` / `cancelled` / `cancel`，并把调用方初始字段放进 `data`。无显式 `parent` 时挂到当前 scheduler 事件。

| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_event(initial: Dictionary = {}) -> GameEvent` | `initial` 调用方传入的初始字段 | 新构建的 `GameEvent` |

- 默认：`trigger_name`（`""`）、`cancelled`（`false`）、`cancel`（`Callable`，内部 `mark_cancelled`）
- `initial` 字段写入 `data`（`parent` 用于挂树，不进 payload）

> 各流程工厂方法在此基础上追加流程专属字段，返回类型均为 `GameEvent`。
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static cancel(event: Variant) -> void` | `event` 为 Dictionary 或 `GameEvent` | 无 |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static is_cancelled(event: Variant) -> bool` | `event` 为 Dictionary 或 `GameEvent` | 任一侧已取消则为真 |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static set_trigger_name(event: Variant, trigger_name: String) -> void` | `event` 为 Dictionary 或 `GameEvent`；`trigger_name` 触发名 | 无 |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_damage_event(target: Entity, source: Entity, num: int, type: Variant, card: Card = null) -> GameEvent` | `target` 受伤实体；`source` 伤害来源；`num` 伤害值；`type` 伤害类型；`card` 武器牌 | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_recover_event(player: Variant, num: int, source: Variant = null) -> GameEvent` | `player` 回复玩家；`num` 回复值；`source` 治疗来源（可 null） | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_move_event(player: Variant, source_block: MapBlock, target_block: MapBlock) -> GameEvent` | `player` 移动玩家；`source_block` 离开地块；`target_block` 进入地块 | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_draw_game_card_event(player: Variant, num: int) -> GameEvent` | `player` 抓牌玩家；`num` 抓牌数 | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_draw_scavenge_event(player: Variant, pile: Pile, num: int) -> GameEvent` | `player` 抓牌玩家；`pile` 拾荒牌堆；`num` 抓牌数 | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_draw_monster_event(player: Variant, num: int) -> GameEvent` | `player` 抓牌玩家；`num` 抓牌数 | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_discard_event(player: Variant, cards: Array, num: int = 1) -> GameEvent` | `player` 弃牌玩家；`cards` 卡牌数组；`num` 弃牌数 | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_monster_death_event(target: Entity, source: Entity) -> GameEvent` | `target` 死亡的怪物；`source` 击杀者 | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_player_death_event(target: Variant, source: Variant) -> GameEvent` | `target` 死亡的玩家；`source` 击杀者（可 `null`） | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_equip_event(player: Variant, card: Card) -> GameEvent` | `player` 装备玩家；`card` 装备牌 | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_consume_charge_event(player: Variant, equipment: Variant, num: int) -> GameEvent` | `player` 玩家；`equipment` 装备；`num` 消耗数 | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_sneak_judge_event(player: Variant, sneak_value: int, block: Variant = null) -> GameEvent` | `player` 检定玩家；`sneak_value` 潜行阈值；`block` 所在地块 | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_spawn_judge_event(player: Variant) -> GameEvent` | `player` 检定玩家 | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_destroy_block_event(source: Variant, block: MapBlock) -> GameEvent` | `source` 摧毁者（可 `null`）；`block` 被摧毁的地块 | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_objective_mark_event(player: Variant, block: MapBlock, mark: Dictionary) -> GameEvent` | `player` 触发玩家；`block` 所在地块；`mark` ObjectiveMark 结构 | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_active_skill_event(player: Variant, targets: Array) -> GameEvent` | `player` 主动技能玩家；`targets` 目标列表 | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_game_start_event(player: Variant) -> GameEvent` | `player` 玩家 | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_game_over_event(player: Variant, result: int) -> GameEvent` | `player` 玩家；`result` 游戏结果（`GameResult.WIN` / `GameResult.LOSE`） | GameEvent |
...
| 签名 | 参数 | 返回 |
|------|------|------|
| `static create_monster_act_event(monster: Monster) -> GameEvent` | `monster` 行动怪物 | GameEvent |
...
| [EventScheduler.md](EventScheduler.md) | 每局调度器：领域操作、输入请求、GameEvent 运行栈 |
| [System/EventBus.md](../System/EventBus.md) | EventBus 信号总线，供 UI 层订阅游戏事件 |
| [04_事件流与变体.md](../../GameInstructions/04_事件流与变体.md) | GameInstructions 侧的事件流程汇总，本文为其 Dictionary schema 源定义 |
