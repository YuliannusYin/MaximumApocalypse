...
- **map_layout 二维数组编码**：`Array[Array[int]]`，值 0/1/2/3 表达空位/任意地块/面包车/必需地块占位（D3，引用 09-MapBlock.md）。
...
> 地块填充算法、地块类型枚举、`is_revealed` 字段见 09-MapBlock.md。
...
> **D101**：原版任务 2"到达警察局第一名玩家抓首领卡"为文本错误，已删除；任务 2 首领仅通过 `SHUFFLE_BOTTOM` 机制入场。
...
> 目标标记的放置算法（满足约束条件）由 `GameSession` 在地图生成阶段执行，详见 09-MapBlock.md。
...
## 10. 任务专属规则与物资（D102-D109 / D111）

### 10.1 科学家装备位置（D102 / D111）

任务 3/8/9 的"科学家"为任务专属装备：
- **来源**：`CardSource.MISSION`，由任务系统直接发放，**不进拾荒牌堆**（D111）。
- **初始位置**：游戏开始时装备在**第一个进行回合的玩家**的装备区（D102）。
- 科学家占用 1 格装备栏，效果为"装备后不可潜行穿过怪物标记"（`scientist_no_stealth`，见 04-ScavengerCard.md 第 7.3 节）。

### 10.2 任务 4 食物交付条件（D103）

任务 4"核冬天"胜利条件中"食物增加到 5"指：
- 队伍持有的**食物类拾荒卡**（`scavenger_color == ScavengerColor.GREEN`，即所有 `food_*` 卡）数量 **≥ 5**。
- 计数范围包括所有存活玩家的手牌区与装备区（即"区域内"，见 D87）。
- 任务 4 的 3 燃料为交付物（R4），`fuel_required = 0`。

### 10.3 任务 6 核辐射扩散"外围"定义（D104）

任务 6 特殊规则"投出 7 时展示/移除外围地块"：
- "外围" = 以面包车为起点做 BFS，**最外层非空地块**（即 BFS 层数最大的地块集合）。
- 当存在多个最外层地块时，这些地块全部视为"外围"。
- 被移除的外围地块：上面的玩家随机移动到周围相邻地块，其他东西（怪物标记、拾荒卡等）直接移出游戏（同 D108 地块摧毁规则）。

### 10.4 任务 8 起始位置（D105）

任务 8 玩家从**军事基地**地块开始（`starting_block = &"military_base"`），该军事基地地块 `is_revealed = true`。

### 10.5 任务 9 "现场"位置（D106）

任务 9 胜利条件中的"现场"（上传病毒地点）是**已摧毁的两个外星发射器地块之一**。玩家到达其中任一块地块即可触发上传病毒的胜利结算。

### 10.6 任务 10 目标标记奖励来源（D107）

任务 10 的 3 个目标标记奖励：
- 全部奖励由**任务系统直接发放**（`CardSource.MISSION`），不消耗拾荒牌堆。
- 奖励内容按原版任务卡描述配置在 `TargetMarkerConfig.on_first_arrival_skill` 对应的 Skill 中。

### 10.7 任务 12 地块摧毁（D108）

任务 12 特殊规则中地块被摧毁时：
1. 该地块上的所有玩家随机移动到周围相邻地块（多个相邻地块时等概率随机）。
2. 该地块上的其他内容（怪物标记、掉落卡、任务物品等）**直接移出游戏**。
3. 被摧毁的地块本身从 `MapGrid` 中 `remove_block()`。

> 详见 09-MapBlock.md 第 2 节 `MapBlockInstance.destroy()` 方法。

### 10.8 重复地图块清单（D109）

任务 2/3、8/9、11/12 使用完全相同的地图块清单是**有意设计**，数据录入阶段直接复用同一组 `map_layout` + `required_blocks` 配置即可。

### 10.9 拾荒卡数量权威来源（D110）

原版卡包未标注具体数量，PC 版以本文档各任务 `scavenger_decks` 配置为**权威来源**。数据录入时按本节配置生成 `.tres` 任务卡，不再对照原版卡包数量。
...
| 2 | 任务 6 核辐射扩散机制"外围"定义 | **已决（D104）**：以面包车为起点 BFS，最外层非空地块为"外围"。详见第 10.3 节 |
| 3 | 任务 10 多目标标记效果奖励来源 | **已决（D107）**：标记 1/2/3 奖励均由任务系统直接发放（`CardSource.MISSION`），不消耗拾荒牌库。详见第 10.6 节 |
...
| 5 | 任务 5/8 的 `trigger_block = &""` 表示"目标地块"的运行时解析 | **已决（D101/D107 相关）**：任务 5/8 首领由 `boss_config` 机制触发，`trigger_block` 在目标标记放置后由任务系统解析回填 |
...
- D101 任务 2"警察局触发首领"为文本错误，已删除
- D102 科学家初始装备在第一个进行回合的玩家装备区
- D103 任务 4"食物增加到 5" = 食物类拾荒卡数量 ≥ 5
- D104 任务 6"外围" = 以面包车为起点 BFS 最外层非空地块
- D105 任务 8 玩家从军事基地开始
- D106 任务 9"现场" = 已摧毁的两个外星发射器地块之一
- D107 任务 10 目标标记奖励由任务系统直接发放
- D108 任务 12 地块摧毁：玩家随机移到相邻地块，其余移出游戏
- D109 任务 2/3、8/9、11/12 重复地图块清单为有意设计
- D110 拾荒卡数量以本文档 `scavenger_decks` 配置为权威来源
- D111 科学家来源 = `MISSION`，不进拾荒牌堆
