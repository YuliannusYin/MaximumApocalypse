...
> 地块的 `monster_spawn_value` 字段见 08-MapBlock.md（待编写）。骰子机制与投骰细节见战斗系统文档（待编写）。
...
> `ON_MONSTER_ATTACK` 与 `ON_STEALTH_CHECK` 是怪物系统新增的触发时机（待 01-Skill.md TriggerTiming 枚举扩展确认，见 Q3/Q6）。
...
| # | 问题 | 决策/临时方案 |
|---|---|---|
| 1 | 僵尸潜行者天赋"血量最低玩家"多个玩家血量相同时的目标选择 | 倾向当前回合玩家优先 → 座次顺序最早者 |
...
| 4 | 外星飞船"不能被射程为短距离的武器或行动选为目标"的实现方式 | **待后续决定**：候选方案 A=新增 `ON_TARGET_SELECT` 时机 + forced=true TRIGGER 拦截 / B=在攻击结算层加 condition 检查（`if weapon.range == SHORT: return false`）。倾向 B（ponytail，最少新时机） |
| 5 | 外星科学家"所有外星人类怪物攻击时伤害 +1"的光环机制 | **待后续决定**：候选方案 A=每个外星怪物 ON_MONSTER_ATTACK 时检查场上有无外星科学家 + 修改伤害 / B=外星科学家自身持有 ON_MONSTER_ATTACK 被动光环（修改所有外星怪物伤害）。倾向 A（数据驱动，避免光环依赖） |
