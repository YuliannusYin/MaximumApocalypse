...
```text
# === 生命与饥饿 ===
var current_hunger: int = 1                       # 当前饥饿等级（初始 1，下限 1）
...
| 字段 | 取值范围 | 变化规则 |
|---|---|---|
| `current_hunger` | ≥ 1（下限 1） | +1（回合/野地夹克）/ -N（食物卡/绿洲/特殊效果）；低于 1 按 1 处理 |
...
- 饥饿值下限为 **1**（不能降到 0 以下）
- `decrease_hunger(amount)` 内部：`current_hunger = max(1, current_hunger - amount)`
...
```text
check_hunger_state():
    if current_hunger ≥ 6 and not is_hungry:
        # 进入饥饿状态
        is_hungry = true
    ...
    elif current_hunger ≤ 5 and is_hungry:
        # 退出饥饿状态
        is_hungry = false
...
当饥饿值达到 6 时（`check_hunger_state` 检测到 `current_hunger ≥ 6 and not is_hungry`）：
1. `is_hungry = true`
...
func can_trigger(event, owner) -> bool:
    return owner is Survivor and owner.current_hunger > 1   # 已达下限不再恢复
...
TurnManager 阶段 4b（回合结束地块效果）：
    对当前回合玩家所在地块：
        if block.block_type == OASIS:
            ...
            #   can_trigger: owner.current_location == oasis block
...
野地夹克用 `extra_actions` 字段分离追踪，与 `actions_remaining` 分开（07-CharacterCard Q2 倾向）。
...
func can_trigger(event, owner) -> bool:
    return owner is Survivor \
        ...
        and owner.current_hunger < 6             # 饥饿 <6 才能用
...
玩家执行行动时：
    if extra_actions > 0:
        ...
        actions_remaining -= 1                   # 再消耗 actions_remaining
```
...
- 玩家 A 回合行动 1：消耗 1 actions_remaining（标准行动点）
- 玩家 A 主动激活野地夹克：
...
- 玩家 A 回合行动 3：消耗 1 actions_remaining
- 玩家 A 主动激活野地夹克：
...
| 时机 | 触发条件 | 用途 |
|---|---|---|
| ON_HUNGER | `increase_hunger` 内 `current_hunger += amount` 后 | 野地夹克等需要在 +1 时介入的技能（实际野地夹克是 ACTIVE 主动技能，不走此时机；预留扩展） |
...
### 12.5 与 MapBlock（08-MapBlock.md，待编写）

- **绿洲地块**：block_type == OASIS，地块被动 Skill 监听 ON_TURN_END_ON_BLOCK
- **玩家所在地块查询**：`survivor.current_location` 用于 ON_TURN_END_ON_BLOCK 触发判定
...
| 信号 | 触发时机 |
|---|---|
| hunger_changed | `current_hunger` 变化时（increase/decrease 内 emit） |
...
本文档新增的决策（待同步到 00-总览与设计决策.md）：

- **D83** 饥饿值下限为 1（多余恢复浪费）
- **D84** 食物箱"全队"范围仅包括存活玩家
- **D85** 食物卡 = Skill（每张食物卡一个 ACTIVE Skill，forced=false）
- **D86** 饥饿相关 TriggerTiming 新增 4 个：ON_HUNGER / ON_HUNGER_STATE_ENTER / ON_HUNGER_STATE_EXIT / ON_HUNGER_DAMAGE
- **D87** 能量饮料免疫用 `hunger_damage_immune` 字段 + SkillExecutor 在 ON_HUNGER_DAMAGE 调度前检查（不引入 event.cancel）
- **D88** 能量饮料时效：使用者下个回合开始时清除未消耗的免疫
- **D89** 野地夹克 +1 走标准 increase_hunger（触发完整 ON_HUNGER 系列时机）
- **D90** 野地夹克恢复的行动数用 `extra_actions` 字段分离追踪，消耗顺序：先 extra_actions 后 actions_remaining
- **D91** 绿洲等地块结束效果用 ON_TURN_END_ON_BLOCK 时机（TriggerTiming 枚举新增）
- **D92** 饥饿伤害走 `take_damage(amount, HUNGER, unavoidable=true)` 入口（跳过 ON_DAMAGE_TAKEN 减伤调度）
- **D93** "进入饥饿本回合不扣血"用 `hunger_damage_this_turn` 标记实现（on_turn_start 重置）
- **D94** increase_hunger 内部触发顺序：ON_HUNGER → check_hunger_state（含 ON_HUNGER_STATE_ENTER/EXIT）→ ON_HUNGER_DAMAGE
- **D95** hunger_damage_sequence 字段语义：值=2/4/6/8/10（10=死亡），扣血后 +=2，退出饥饿重置回 2（修正 07-CharacterCard.md 注释）
