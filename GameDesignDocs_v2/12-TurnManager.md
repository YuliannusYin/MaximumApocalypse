...
# === 行动点 ===
var action_points: int = 0               # 当前剩余行动点
...
> 回合阶段按顺序执行：MONSTER_SPAWN → DRAW_CARD → ACTION → TURN_END → FINISHED。阶段 3（ACTION）由玩家 UI 驱动，玩家执行行动或点击"结束回合"按钮后进入阶段 4。
...
玩家有 4 个行动点（`MAX_ACTION_POINTS = 4`），可按任意组合执行行动，可多次执行同一行动。行动点不可累积，回合结束未用完的行动点作废。
...
| 行动 | 方法签名 | 规则要点 |
|---|---|---|
| **移动** | `move_to_adjacent_block(dir: Direction)` | 横/竖向移动 1 格到相邻地块；不能斜向。进入时展示地块并执行地块效果（ON_REVEAL + ON_ENTER）。若进入有怪物标记的地块且自己面前无怪物，需做潜行检定 |
...
### Q1. Survivor.current_block 字段

TurnManager 假设 Survivor 有 `current_block: MapBlockInstance` 字段表示当前所在地块。待 07-CharacterCard.md 确认是否已定义此字段，或通过 `game_session.map_grid.get_block(survivor.grid_pos)` 查询。

### Q2. Survivor.engaged_monsters 字段

TurnManager 假设 Survivor 有 `engaged_monsters: Array[Monster]` 字段表示面前的纠缠怪物列表。待 07-CharacterCard.md / 05-MonsterCard.md 确认字段名。
...
某些效果可增减行动点（如监狱 ON_ENTER "本回合行动点 -1"、野地夹克恢复行动数）。这些效果的实现方式：
- 选项 A：直接修改 `action_points` 字段
- 选项 B：通过 `add_action(n)` / `reduce_action(n)` 方法（便于 UI 通知）
- 倾向 B（保持 action_points_changed 信号触发）
